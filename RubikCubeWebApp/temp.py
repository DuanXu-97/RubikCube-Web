...
def gen_testcase():
    with open(os.path.join(BASE_DIR, 'RubikCubeWebApp/solver/tests/test_states_by_converted_id.pkl'), "rb") as f:
        states_by_id = pickle.load(f)

    states_by_face = []
    for state in states_by_id:
        state_by_face = convert_states_to_str(state)
        states_by_face.append(state_by_face)
        print(state_by_face)

    with open(os.path.join(BASE_DIR, 'RubikCubeWebApp/solver/tests/test_states_by_face.pkl'), 'wb') as f_w:
